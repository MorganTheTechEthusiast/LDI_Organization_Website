    ...
    <Link to={`/programs/${item.id}`} className="group block h-full">
      <article className="card relative h-full overflow-hidden">
        <div className="relative overflow-hidden">
          <CardImage src={item.image_url} alt={item.title} className="h-60 transition duration-500 group-hover:scale-105" />
        ...
        <h3 className="font-heading text-2xl font-black leading-tight text-ink transition group-hover:text-maroon dark:group-hover:text-ember">{item.title}</h3>
